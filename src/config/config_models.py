...
class RateLimitConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.capacity, int):
            raise ValueError(
                f"Rate limit capacity must be an integer, got {type(self.capacity).__name__}"
            )
        if self.capacity <= 0:
            raise ValueError(
                f"Rate limit capacity must be positive, got {self.capacity}"
            )
        if self.capacity > 10000:
            raise ValueError(
                f"Rate limit capacity seems too high ({self.capacity}), maximum recommended is 10000"
            )

        if not isinstance(self.refill_rate, int):
            raise ValueError(
                f"Rate limit refill_rate must be an integer, got {type(self.refill_rate).__name__}"
            )
        ...
        if not isinstance(self.initial_tokens, int):
            raise ValueError(
                f"Initial tokens must be an integer, got {type(self.initial_tokens).__name__}"
            )
        if self.initial_tokens < 0:
            raise ValueError(
                f"Initial tokens must be non-negative, got {self.initial_tokens}"
            )
        if self.initial_tokens > self.capacity:
            raise ValueError(
                f"Initial tokens ({self.initial_tokens}) cannot exceed capacity ({self.capacity})"
            )

        if not isinstance(self.safety_margin, (int, float)):
            raise ValueError(
                f"Safety margin must be a number, got {type(self.safety_margin).__name__}"
            )
        if not 0.0 <= self.safety_margin <= 1.0:
            raise ValueError(
                f"Safety margin must be between 0.0 and 1.0, got {self.safety_margin}"
            )
    ...
    def __post_init__(self) -> None:
        ...
        for field, value in self.__dict__.items():
            if not isinstance(value, int):
                raise ValueError(
                    f"Pagination size for {field} must be an integer, got {type(value).__name__}"
                )
            if value <= 0:
                raise ValueError(
...
class DelayConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.page_delay, (int, float)):
            raise ValueError(
                f"Page delay must be a number, got {type(self.page_delay).__name__}"
            )
        ...
        if not isinstance(self.request_timeout, (int, float)):
            raise ValueError(
                f"Request timeout must be a number, got {type(self.request_timeout).__name__}"
            )
        ...
        if self.request_timeout > 300:
            raise ValueError(
                f"Request timeout seems too high ({self.request_timeout}s), maximum recommended is 300s"
            )

        if not isinstance(self.retry_base_delay, (int, float)):
            raise ValueError(
                f"Retry base delay must be a number, got {type(self.retry_base_delay).__name__}"
            )
        if self.retry_base_delay <= 0:
            raise ValueError(
                f"Retry base delay must be positive, got {self.retry_base_delay}"
            )
        if self.retry_base_delay > 60:
            raise ValueError(
                f"Retry base delay seems too high ({self.retry_base_delay}s), maximum recommended is 60s"
            )
...
class BackoffConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.initial_delay, (int, float)):
            raise ValueError(
                f"Initial delay must be a number, got {type(self.initial_delay).__name__}"
            )
        if self.initial_delay <= 0:
            raise ValueError(
                f"Initial delay must be positive, got {self.initial_delay}"
            )
        if self.initial_delay > 60:
            raise ValueError(
                f"Initial delay seems too high ({self.initial_delay}s), maximum recommended is 60s"
            )

        if not isinstance(self.max_delay, (int, float)):
            raise ValueError(
                f"Max delay must be a number, got {type(self.max_delay).__name__}"
            )
        if self.max_delay <= 0:
            raise ValueError(f"Max delay must be positive, got {self.max_delay}")
        if self.max_delay > 3600:
            raise ValueError(
                f"Max delay seems too high ({self.max_delay}s), maximum recommended is 3600s (1 hour)"
            )
        if self.max_delay < self.initial_delay:
            raise ValueError(
                f"Max delay ({self.max_delay}) must be >= initial delay ({self.initial_delay})"
            )

        if not isinstance(self.multiplier, (int, float)):
            raise ValueError(
                f"Multiplier must be a number, got {type(self.multiplier).__name__}"
            )
        if self.multiplier <= 1.0:
            raise ValueError(f"Multiplier must be > 1.0, got {self.multiplier}")
        if self.multiplier > 10.0:
            raise ValueError(
                f"Multiplier seems too high ({self.multiplier}), maximum recommended is 10.0"
            )

        if not isinstance(self.jitter_factor, (int, float)):
            raise ValueError(
                f"Jitter factor must be a number, got {type(self.jitter_factor).__name__}"
            )
        if not 0.0 <= self.jitter_factor <= 1.0:
            raise ValueError(
                f"Jitter factor must be between 0.0 and 1.0, got {self.jitter_factor}"
            )
