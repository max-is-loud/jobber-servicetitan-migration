...
class TimesheetEntriesMapExtractor(BaseMapExtractor):
    """Map mode extractor for TimesheetEntry entities - lightweight discovery pass."""
    ...
    def _fetch_page(self, cursor: Optional[str] = None, page_size: Optional[int] = None) -> dict[str, Any]:
        """Fetch a page of timesheet entries using map mode query."""
        return self._jobber_client.fetch_timesheet_entries_map(cursor, page_size)
    ...
    def _map_entity(self, node: dict[str, Any]) -> EntityInventory:
        """Map a timesheet entry node to EntityInventory."""
        return EntityInventory(
            entity_type="timesheetEntries",
            entity_id=node["id"],
