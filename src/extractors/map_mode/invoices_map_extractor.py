"""InvoicesMapExtractor for lightweight invoice entity discovery."""
...
class InvoicesMapExtractor(BaseMapExtractor):
    """Map mode extractor for Invoice entities - lightweight discovery pass."""
    ...
    def __init__(
        ...
        """Initialize InvoicesMapExtractor.
    ...
    def _fetch_page(self, cursor: Optional[str] = None, page_size: Optional[int] = None) -> dict[str, Any]:
        """Fetch a page of invoices using map mode query."""
        return self._jobber_client.fetch_invoices_map(cursor, page_size)

    def _extract_edges_and_page_info(self, response: dict[str, Any]) -> tuple[List[dict[str, Any]], dict[str, Any]]:
        """Extract edges and page info from invoices API response."""
    ...
    def _map_entity(self, node: dict[str, Any]) -> EntityInventory:
        """Map an invoice node to EntityInventory with relation counts."""
